...
{
    public partial class FormBuscarProfessor : Form
    {
        public FormBuscarProfessor()
        {
            InitializeComponent();
        }
        private void buttonBuscar_Click(object sender, EventArgs e)
        {
            try
            {
                usuarioBindingSource.DataSource = new ProfessorBLL().BuscarTodos();
            }
            catch (Exception ex)
            {
        ...
        private void buttonExcluirUsuario_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int id = ((Professor)usuarioBindingSource.Current).Id;
                new ProfessorBLL().Excluir(id);
        ...
        {
            using (FormCadastroProfessor frm = new FormCadastroProfessor())
            {
                frm.ShowDialog();
            }
        ...
        private void buttonAlterar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (FormCadastroProfessor frm = new FormCadastroProfessor(id))
                {
                    frm.ShowDialog();
                }
                buttonBuscar_Click(null, null);
        ...
        private void buttonAdicionarGrupoUsuario_Click(object sender, EventArgs e)
        {
            try
            {
                using (FormConsultaGrupoUsuario frm = new FormConsultaGrupoUsuario())
                {
                    ...
                    int idUsuario = ((Professor)usuarioBindingSource.Current).Id;
                    int idGrupoUsuarioSelecionado = frm.IdGrupoUsuarioSelecionado;

                    if (idGrupoUsuarioSelecionado != 0)
                    {
                        new ProfessorBLL().AdicionarGrupoUsuario(idUsuario, idGrupoUsuarioSelecionado);
                    }
                }
                buttonBuscar_Click(null, null);
            }
        ...
        private void buttonExcluirGrupoUsuario_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int idUsuario = ((Professor)usuarioBindingSource.Current).Id;
                new ProfessorBLL().RemoverGrupoUsuario(idUsuario, idGrupoUsuario);
        ...
        private void usuarioDataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (e.RowIndex >= 0)
                {
                    // Obtenha o objeto Professor da linha selecionada.
                    Professor usuarioSelecionado = (Professor)usuarioBindingSource.List[e.RowIndex];
